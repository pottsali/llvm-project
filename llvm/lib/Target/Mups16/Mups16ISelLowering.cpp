                ...
                SmallVectorImpl<SDValue> &InVals) const
{
    ...
    // Analyze operands of the call, assigning locations to each operand. Each
    // of the CCValAssign instances represents a single argument, and will
    // include information on whether the size of the source value, whether it
    // needs sign extension, etc. (for example, if we pass a signed char via a
    // 16-bit register)
    // CC_Mups16 is a function that's auto-generated by tablegen from
    // Mups16CallingConv.td
    SmallVector<CCValAssign, 16> ArgLocs;
    CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs, *DAG.getContext());
    CCInfo.AnalyzeCallOperands(Outs, CC_Mups16);

    // Get the size of the outgoing arguments stack space requirement.
    unsigned ArgsSize = CCInfo.getNextStackOffset();

    // Keep stack frames 2-byte aligned.
    ArgsSize = (ArgsSize+1) & ~1;

    MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();

    Chain = DAG.getCALLSEQ_START(Chain, ArgsSize, 0, DL);

    // Sequence of arguments that will be passed in registers
    ...
    for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i)
    {
        ...
        // TODO: handle byval args (pointer args that need to be copied before
        // calling, so the callee gets a pointer to the copy, not the original)
        if (Flags.isByVal())
        {
            report_fatal_error("byval args not supported yet");
        }
        ...
        {
            default:
                llvm_unreachable("Non-full argument passing not supported yet");
        ...
        {
            assert(VA.isMemLoc());
            llvm_unreachable("Passing arguments on stack not supported yet");
        }
    ...
    // The glue is passed to the JAL/JALR instruction so that the nodes setting
    // up the stack above are not separated from the call, and the nodes to
    // process the return value are similarly glued after the call.
    SDVTList Glue = DAG.getVTList(MVT::Other, MVT::Glue);
    ...
    Chain = DAG.getNode(Mups16ISD::JmpLink, DL, Glue, Ops);
    InFlag = Chain.getValue(1);
